...
# 숫자 변환 안전 함수 (문자가 섞여도 숫자만 추출, 실패 시 0)
def to_safe_float(series):
    # 1. 문자열로 변환 후 숫자와 소수점 외의 모든 문자 제거
    cleaned = series.astype(str).str.replace(r'[^0-9.]', '', regex=True)
    # 2. 숫자로 변환 (변환 불가한 'J16Y012' 등은 NaN이 됨)
    # 3. NaN은 0으로 채움
...
if uploaded_file:
    try:
        ...
        # 1. 상품 코드(MECODE) 처리: 무조건 문자열(Object)로 취급하여 'J16Y012' 에러 방지
        ...
        # 2. 수량/환산 데이터 처리: 숫자 외 문자 제거 후 float 변환
